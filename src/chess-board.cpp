...
ChessBoard::getOverlap(const std::vector<std::pair<int, int>> &el1,
...
ChessBoard::getDangerousPoints(ChessPieceBase ***board, bool white) {
...
void ChessBoard::printBoard(bool white, std::ostream *out = &std::cout,
                            bool server = false) {
  int i, j;
  ...
  if (depth == maxDepth) {
    ...
    for (i = 0; i < out.size(); ++i) {
      ...
      if (i == 0) {
        maxScore =
            out.at(i).dScore -
            worth * recursiveSubroutine(imaginaryBoard, !white, difficulty - 1,
...
float ChessBoard::performMove(const Move &move, ChessPieceBase ***board,
  ...
  if (board != nullptr) {
    ...
    if (overrideRightess) {
      if (board[move.start.first][move.start.second]->isWhite() !=
              board[move.end.first][move.end.second]->isWhite() &&
        ...
        if (board[move.start.first][move.start.second]->getCode() == PAWN &&
          ...
          buf = createPeice(
              ...
              board[move.start.first][move.start.second]->getLogger(), board,
              true);
        ...
        if (board[move.end.first][move.end.second]->getCode() != ROOK) {
          ...
          for (std::pair<int, int> coord :
               board[move.start.first][move.start.second]->getAttackCandidates(
                   true)) {
            score -= getScore(board[coord.first][coord.second]->getCode()) *
                     ATTACK_COST;
          }
          if (!board[move.start.first][move.start.second]->hasMoved() &&
              board[move.start.first][move.start.second]->getCode() != KING &&
          ...
          if (board[move.start.first][move.start.second]->getCode() == PAWN &&
            ...
            board[move.end.first][move.end.second] = buf;
            for (std::pair<int, int> coord :
                 board[move.end.first][move.end.second]->getAttackCandidates(
                     true)) {
              score += getScore(board[coord.first][coord.second]->getCode()) *
                       ATTACK_COST;
            }
          ...
          for (std::pair<int, int> coord :
               board[bufMoveKing.first][bufMoveRook.second]
                   ->getAttackCandidates(true)) {
            score += getScore(board[coord.first][coord.second]->getCode()) *
                     ATTACK_COST;
          }
      ...
      if (board[move.start.first][move.start.second]->canAttack(move.end)) {
        ...
        board[move.start.first][move.start.second] = new ChessPieceEmpty(
            ...
            board[move.end.first][move.end.second]->getLogger(), board);
        return score;
      } else if (board[move.start.first][move.start.second]->canMoveTo(
                     move.end)) {
